...
// ─── Page ─────────────────────────────────────────────────────────────────────
export function ROCAUCAdvancedLab() {
  ...
  const comparisonSeries = useSeriesVisibility([
    "diagonal",
    "good",
    "medium",
    "bad",
  ...
  const loanPr = useMemo(
    () => precisionRecallCurve(LOAN_ACTUAL, LOAN_SCORES),
    [],
  );
  ...
  const operatingPoint = useMemo(() => {
    ...
    return {
      fpr: loanRoc.fpr[bestIdx],
      tpr: loanRoc.tpr[bestIdx],
      idx: bestIdx,
    };
  }, [loanRoc, threshold]);
  ...
  const goodRoc = useMemo(() => {
    const s = syntheticScores(50, "good");
    return rocCurve(s.actual, s.scores);
  }, []);
  const mediumRoc = useMemo(() => {
    const s = syntheticScores(50, "medium");
    return rocCurve(s.actual, s.scores);
  }, []);
  const badRoc = useMemo(() => {
    const s = syntheticScores(50, "bad");
    return rocCurve(s.actual, s.scores);
  }, []);
  ...
  const comparisonData = useMemo(() => {
    const g = goodRoc.fpr.map((f, i) => ({
      fpr: parseFloat(f.toFixed(3)),
      good: parseFloat(goodRoc.tpr[i].toFixed(3)),
    }));
    const m = mediumRoc.fpr.map((f, i) => ({
      fpr: parseFloat(f.toFixed(3)),
      medium: parseFloat(mediumRoc.tpr[i].toFixed(3)),
    }));
    const b = badRoc.fpr.map((f, i) => ({
      fpr: parseFloat(f.toFixed(3)),
      bad: parseFloat(badRoc.tpr[i].toFixed(3)),
    }));
    // Pad to same length
  ...
  const OpDot = (props: {
    cx?: number;
    cy?: number;
    payload?: { fpr?: number; tpr?: number };
  }) => {
    const { cx, cy, payload } = props;
    if (cx === undefined || cy === undefined) return null;
    const isOp =
      Math.abs((payload?.fpr ?? -1) - operatingPoint.fpr) < 0.015 &&
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      <Card title="Classification Threshold">
        <div className="flex items-center gap-4">
          ...
          />
          <span className="text-lg font-black text-blue-600 w-14 text-right">
            {threshold.toFixed(2)}
          </span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          {[
            ...
            <div
              key={label}
              className="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/40 text-center"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <Card
            title={`${curveMode === "roc" ? "ROC Curve" : "Precision-Recall Curve"} - Loan Dataset`}
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span
                  className={`text-2xl font-black ${loanRoc.auc >= 0.9 ? "text-green-600" : loanRoc.auc >= 0.8 ? "text-blue-600" : "text-orange-600"}`}
                >
                ...
                <span
                  className={`text-sm font-medium px-2 py-0.5 rounded-full ${loanRoc.auc >= 0.9 ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}
                >
                  {loanRoc.auc >= 0.9
                    ? "Excellent"
                    : loanRoc.auc >= 0.8
            ...
            {curveMode === "roc" ? (
              <>
                <ResponsiveContainer width="100%" height={320}>
                  ...
                  >
                    ...
                    <XAxis
                      dataKey="fpr"
                      type="number"
                      domain={primaryZoom.xDomain ?? [0, 1]}
                      label={{
                    ...
                    <Tooltip
                      ...
                      labelFormatter={(v) =>
                        `FPR: ${(Number(v) * 100).toFixed(1)}%`
                      }
                    />
                    <Legend
                      onClick={primarySeries.legendClick}
                      wrapperStyle={{ color: palette.axis, cursor: "pointer" }}
                    />
                    ...
                    {primaryZoom.refAreaLeft !== null &&
                      primaryZoom.refAreaRight !== null && (
                        <ReferenceArea
                          x1={primaryZoom.refAreaLeft}
                          x2={primaryZoom.refAreaRight}
                          strokeOpacity={0.3}
                ...
                <p className="text-xs text-gray-500 mt-2 text-center">
                  Dashed line = random classifier (AUC=0.5). Area under blue
                  curve = AUC.
                </p>
              </>
            ) : (
              <>
                ...
                <InfoBox title="When to read the PR curve" type="info">
                  ...
                  datasets. A random classifier gives AP equal to the positive
                  class prevalence ({(classPrevalence * 100).toFixed(1)}%), not
                  50%.
                </InfoBox>
              </>
            )}
        ...
        <Card
          ...
          subtitle={`At threshold = ${threshold.toFixed(2)}`}
        >
          <div className="grid grid-cols-2 gap-1 mb-4">
            {[
              ...
              },
              {
                ...
                value: tn,
                color:
                  "bg-blue-100  dark:bg-blue-900/40  text-blue-800  dark:text-blue-200  border-blue-300",
              },
            ].map(({ label, value, color }) => (
              <div
                key={label}
                className={`flex flex-col items-center justify-center p-4 rounded-lg border ${color}`}
              >
      ...
      {/* Comparison toggle */}
      <Card
        title="Multi-Classifier Comparison"
        actions={
          <button
        ...
        {showComparison ? (
          <div>
            <div className="flex gap-4 mb-3 flex-wrap text-xs">
              {[
                {
                  label: `Good Classifier   AUC=${goodRoc.auc.toFixed(3)}`,
                  color: palette.series[1],
                },
                {
                  label: `Medium Classifier AUC=${mediumRoc.auc.toFixed(3)}`,
                  color: palette.series[4],
                },
                {
                  label: `Bad Classifier    AUC=${badRoc.auc.toFixed(3)}`,
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <XAxis
                  dataKey="fpr"
                  type="number"
                  domain={comparisonZoom.xDomain ?? [0, 1]}
                  label={{
                    value: "FPR",
                    position: "insideBottom",
                    offset: -12,
                ...
                />
                <Legend
                  onClick={comparisonSeries.legendClick}
                  wrapperStyle={{ color: palette.axis, cursor: "pointer" }}
                />
                {!comparisonSeries.hidden.has("diagonal") && (
                  <Line
                    data={diagonal}
                    dataKey="diagonal"
                    dot={false}
                ...
                {!comparisonSeries.hidden.has("good") && (
                  <Line
                    dataKey="good"
                    dot={false}
                ...
                {!comparisonSeries.hidden.has("medium") && (
                  <Line
                    dataKey="medium"
                    dot={false}
                    stroke={palette.series[4]}
                ...
                {!comparisonSeries.hidden.has("bad") && (
                  <Line
                    dataKey="bad"
                    dot={false}
                ...
                {comparisonZoom.refAreaLeft !== null &&
                  comparisonZoom.refAreaRight !== null && (
                    <ReferenceArea
                      x1={comparisonZoom.refAreaLeft}
                      x2={comparisonZoom.refAreaRight}
                      strokeOpacity={0.3}
                      fill={palette.series[0]}
                      fillOpacity={0.12}
                    />
          ...
          <p className="text-sm text-gray-500">
            Click "Show Comparison" to overlay Good / Medium / Bad classifiers
            on the same ROC chart.
          </p>
        )}
      </Card>
      ...
      <Card title="AUC Interpretation Guide">
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {AUC_GUIDE.map(({ range, label, color }) => (
            <div
              key={range}
              className="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/40 text-center border border-gray-200 dark:border-gray-600"
            >
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <InfoBox type="success" title="AUC > 0.90 — Excellent">
            Classifier strongly separates classes. Suitable for high-stakes
            decisions like medical diagnosis or fraud detection.
          </InfoBox>
          <InfoBox type="info" title="AUC = 0.50 — Random Classifier">
            Classifier has no discriminative ability. Its ROC curve follows the
            diagonal. Re-examine features and model.
          </InfoBox>
        </div>
      </Card>
    </div>
