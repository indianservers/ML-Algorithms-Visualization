...
import ConfusionMatrixApprovedPage from "./ConfusionMatrixApprovedPage";
...
const MULTI_PREDICTED = [
  0, 0, 1, 0, 0, 1, 1, 2, 1, 1, 2, 2, 2, 0, 2, 0, 1, 2, 1, 0,
];
const CLASS_LABELS = ["Setosa", "Versicolor", "Virginica"];

function multiConfusionMatrix(
  actual: number[],
  predicted: number[],
  n: number,
): number[][] {
...
// ─── Page component ───────────────────────────────────────────────────────────
export function ConfusionMatrixAdvancedLab() {
  // Manual mode state
  const [manual, setManual] = useState<ManualCM>({
    tp: 50,
    tn: 40,
    fp: 10,
    fn: 8,
  ...
  const manF1 =
    manPrec + manRec > 0 ? (2 * manPrec * manRec) / (manPrec + manRec) : 0;
  ...
  const metricRow = (label: string, value: number, description: string) => (
    ...
    >
      ...
      <span
        className={`text-lg font-bold ${value >= 0.8 ? "text-green-600" : value >= 0.6 ? "text-amber-600" : "text-red-600"}`}
      >
        {(value * 100).toFixed(1)}%
      </span>
    </div>
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      {multiMode ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card
            ...
            subtitle="Iris toy data — rows=Actual, cols=Predicted"
          >
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr>
                    <th className="p-2 text-gray-400 text-xs font-normal">
                      Actual ↓ / Pred →
                    </th>
                    {CLASS_LABELS.map((l) => (
                      <th
                        key={l}
                        className="p-2 text-xs font-semibold text-gray-700 dark:text-gray-300"
                      >
                        {l}
                      </th>
                    ))}
                ...
                <tbody>
                  {CLASS_LABELS.map((rowLabel, ri) => (
                    <tr key={ri}>
                      <td className="p-2 text-xs font-semibold text-gray-700 dark:text-gray-300">
                        {rowLabel}
                      </td>
                      {multiMatrix[ri].map((val, ci) => (
                        ...
                        >
                          <div className="p-3 font-bold text-lg text-gray-900 dark:text-white">
                            {val}
                          </div>
                        </td>
                      ))}
          ...
          <Card title="Per-Class Metrics">
            <div className="space-y-4">
              {CLASS_LABELS.map((label, i) => {
                ...
                const fp_i = multiMatrix.reduce(
                  (s, r, ri) => s + (ri !== i ? r[i] : 0),
                  0,
                );
                const fn_i = multiMatrix[i].reduce(
                  (s, v, ci) => s + (ci !== i ? v : 0),
                  0,
                );
            ...
            <div className="mt-4 text-xs text-gray-500 p-2 bg-gray-50 dark:bg-gray-700/30 rounded">
              Overall Accuracy:{" "}
              {(
                (MULTI_ACTUAL.filter((a, i) => a === MULTI_PREDICTED[i])
                  .length /
                  MULTI_ACTUAL.length) *
                100
              ).toFixed(1)}
              % &nbsp;(
              {MULTI_ACTUAL.filter((a, i) => a === MULTI_PREDICTED[i]).length}/
              {MULTI_ACTUAL.length} correct)
            </div>
          </Card>
        </div>
      ) : (
              ...
              {tab === "manual" ? (
                ...
                <Card title="Enter Confusion Matrix Values">
                  ...
                  <InfoBox type="info">
                    ...
                    &nbsp;|&nbsp;
                    <strong>FP</strong> = predicted positive, actually negative
                    (Type I error) &nbsp;|&nbsp;
                    <strong>FN</strong> = predicted negative, actually positive
                    (Type II error)
                  </InfoBox>
                </Card>
              ) : (
                ...
                <Card title="Loan Dataset — Threshold-based Classifier">
                  ...
                  <p className="text-xs text-gray-500">
                    Score is a weighted heuristic of credit_score, income,
                    debt_ratio, employment_years. Samples: {loanRows.length}
                  </p>
                </Card>
              )}
                    ...
                    const m =
                      tab === "manual"
                        ...
                        : {
                            ...
                            precision: dsMetrics.precision,
                            recall: dsMetrics.recall,
                            specificity: dsMetrics.specificity,
                            f1: dsMetrics.f1,
                          };
                    return (
                      <div className="space-y-0">
                        ...
                        {metricRow(
                          "Precision",
                          m.precision,
                          "Of all predicted +, how many are actually +",
                        )}
                        {metricRow(
                          "Recall",
                          m.recall,
                          "Of all actual +, how many did we catch?",
                        )}
                        {metricRow(
                          "Specificity",
                          m.specificity,
                          "Of all actual −, how many did we correctly reject?",
                        )}
                        {metricRow(
                          "F1-Score",
                          m.f1,
              ...
              <Card title="Metric Formulas">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {FORMULAS.map(({ label, formula }) => (
                    <div
                      key={label}
                      className="rounded-lg bg-gray-50 dark:bg-gray-700/40 p-3 border border-gray-200 dark:border-gray-600"
                    >
      ...
      <InfoBox type="warning" title="When do errors matter most?">
        <ul className="list-disc list-inside space-y-1">
          <li>
            <strong>High Recall critical</strong> — medical diagnosis (minimise
            False Negatives)
          </li>
          <li>
            <strong>High Precision critical</strong> — spam filters (minimise
            False Positives)
          </li>
          <li>
            <strong>Balanced F1</strong> — imbalanced datasets where accuracy
            alone is misleading
          </li>
          <li>
            <strong>Specificity</strong> — screening tests where true-negative
            rate matters
          </li>
        </ul>
      </InfoBox>
...
export default function ConfusionMatrixPage() {
  return new URLSearchParams(window.location.search).has("advanced") ? (
    <ConfusionMatrixAdvancedLab />
  ) : (
    <ConfusionMatrixApprovedPage />
  );
}
