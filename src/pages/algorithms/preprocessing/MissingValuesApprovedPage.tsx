...
export default function MissingValuesApprovedPage(){
  ...
  return <div className={`mv-page ${collapsed?'collapsed':''}`}>
    <aside className="mv-side"><a href="/" className="mv-brand"><span>✦</span><b>mega ML<small>AI OBSERVATORY</small></b></a><button className="home active" onClick={()=>go('Home')}>⌂ Home</button><nav><small>LEARN</small>{['▣ Roadmap','♡ Courses'].map(label=><button key={label} onClick={()=>go(label)}>{label}</button>)}<small>TOPICS</small>{['› Data','› Missing Values','› Outliers','› Encoding','› Scaling','› Feature Engineering'].map((label,index)=><button className={index===1?'active':''} key={label} onClick={()=>go(label)}>{label}</button>)}<small>LAB</small>{['▧ Notebooks','⌘ Experiments','▤ Datasets','◇ Models'].map(label=><button key={label} onClick={()=>go(label)}>{label}</button>)}<small>RESOURCES</small>{['▧ Guides','♧ Cheat Sheets','⌁ Playground'].map(label=><button key={label} onClick={()=>go(label)}>{label}</button>)}</nav><button className="collapse" onClick={()=>setCollapsed(value=>!value)}>{collapsed?'»':'‹'}</button></aside>
