...
import {
  ...
  makeTrainingData,
  sequenceSample,
  type TensorFlowLabMode,
} from "../../../lib/algorithms/neural/tensorflowDeepLearning";

type Mode = TensorFlowLabMode;

const copy: Record<
  Mode,
  { title: string; subtitle: string; inputShape: string; output: string }
> = {
  cnn: {
    title: "CNN",
    subtitle:
      "Real TensorFlow.js convolutional neural network trained on synthetic 8x8 image patterns.",
  ...
  rnn: {
    title: "RNN",
    subtitle:
      "Real TensorFlow.js SimpleRNN sequence classifier trained on synthetic rising/falling signals.",
  ...
  lstm: {
    title: "LSTM",
    subtitle:
      "Real TensorFlow.js LSTM sequence classifier with gated recurrent memory.",
    inputShape: "12 time steps x 1 feature",
    output: "rising vs falling sequence class probability",
  },
  gru: {
    title: "GRU",
    subtitle:
      "Real TensorFlow.js GRU sequence classifier with update/reset gates.",
    inputShape: "12 time steps x 1 feature",
    output: "rising vs falling sequence class probability",
  },
};
...
export default function TensorFlowDeepLearningLab({ mode }: { mode: Mode }) {
  ...
  const preview = useMemo(
    () =>
      mode === "cnn" ? imageSample(101).values : sequenceSample(101).values,
    [mode],
  );
  ...
  const train = async () => {
    ...
    try {
      ...
      output.dispose();
      setStatus(
        "Training complete. Prediction computed from held-out preview sample.",
      );
    } finally {
      data.xs.dispose();
      data.ys.dispose();
      ...
      <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
          ...
          <Card title="TensorFlow.js Controls">
            <div className="space-y-4 text-sm">
              ...
              <button
                disabled={training}
                onClick={train}
                className="inline-flex w-full items-center justify-center gap-2 rounded bg-blue-600 px-3 py-2 font-semibold text-white disabled:opacity-60"
              >
        ...
        <div className="space-y-4">
          <TrainingLossChart
            ...
            subtitle="Updates at each TensorFlow.js epoch while training runs."
            height={340}
          />
          <div className="grid gap-4 lg:grid-cols-2">
            <Card title="Input Preview">
              {mode === "cnn" ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart
                    data={preview.map((value, step) => ({ step, value }))}
                  >
            ...
            <Card title="Model Summary">
              <div className="space-y-2 text-sm text-gray-700 dark:text-gray-200">
                <p>
                  <b>Input:</b> {meta.inputShape}
                </p>
                <p>
                  <b>Layer:</b>{" "}
                  {mode === "cnn"
                    ? "Conv2D × 2 + MaxPool × 2 + Dense"
                    : mode.toUpperCase()}
                </p>
                <p>
                ...
                <p>
                  <b>Prediction:</b>{" "}
                  {prediction
                    ? `class ${prediction.label} at ${(prediction.probability * 100).toFixed(1)}%`
                    : "Train to compute"}
                </p>
              </div>
            </Card>
          ...
          <InfoBox type="info" title="Real TensorFlow.js Implementation">
            This route builds, compiles, trains, predicts, and disposes an
            actual TensorFlow.js model in the browser.
          </InfoBox>
        </div>
      </div>
    </div>
