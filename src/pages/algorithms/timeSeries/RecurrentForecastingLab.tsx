...
const metadata: Record<
  RecurrentMode,
  { title: string; subtitle: string; layer: string; gateNote: string }
> = {
  rnn: {
    title: "RNN Time-Series Forecasting",
    subtitle:
      "Train a SimpleRNN in TensorFlow.js on seasonal browser data and run next-step inference live.",
    layer: "SimpleRNN",
    gateNote:
      "A compact recurrent state carries recent signal context from one time step to the next.",
  },
  lstm: {
    title: "LSTM Time-Series Forecasting",
    subtitle:
      "Train an LSTM forecaster with gated memory, recursive horizon inference, and live loss updates.",
    layer: "LSTM",
    gateNote:
      "Input, forget, and output gates decide what the model keeps, updates, and exposes.",
  },
  gru: {
    title: "GRU Time-Series Forecasting",
    subtitle:
      "Train a GRU forecaster that uses reset/update gates for efficient sequence memory.",
    layer: "GRU",
    gateNote:
      "Update and reset gates give recurrent memory control with fewer parameters than LSTM.",
  },
};
...
function numericColumns(dataset: Dataset) {
  return dataset.columns.filter((column) =>
    dataset.data.some((row) => typeof row[column] === "number"),
  );
}
...
function labelColumn(dataset: Dataset, targetColumn: string) {
  return (
    dataset.columns.find(
      (column) =>
        column !== targetColumn &&
        typeof dataset.data[0]?.[column] !== "number",
    ) ?? dataset.columns[0]
  );
}
...
function buildModel(
  ...
  const recurrentConfig = {
    inputShape: [lookback, 1] as [number, number],
    units,
...
function computeMae(points: SeriesPoint[]) {
  const compared = points.filter(
    (point) =>
      typeof point.actual === "number" && typeof point.fitted === "number",
  );
  if (compared.length === 0) return 0;
  return (
    compared.reduce(
      (sum, point) => sum + Math.abs((point.actual ?? 0) - (point.fitted ?? 0)),
      0,
    ) / compared.length
  );
}
...
function RecurrentForecastingLab({ mode }: { mode: RecurrentMode }) {
  ...
  const selectedDataset =
    options.find((dataset) => dataset.id === selectedDatasetId) ?? options[0];
  const availableTargets = numericColumns(selectedDataset);
  const [targetColumn, setTargetColumn] = useState(
    defaultTargets[options[0].id] ?? availableTargets[0] ?? "",
  );
  const [lookback, setLookback] = useState(quickVerification ? 6 : 12);
  const [units, setUnits] = useState(
    quickVerification ? 4 : mode === "lstm" ? 14 : 10,
  );
  const [epochs, setEpochs] = useState(quickVerification ? 1 : 24);
  const [horizon, setHorizon] = useState(quickVerification ? 3 : 8);
  ...
  const resolvedTarget = availableTargets.includes(targetColumn)
    ? targetColumn
    : availableTargets[0];
  const allDatasetSeries = seriesFromDataset(selectedDataset, resolvedTarget);
  const datasetSeries = quickVerification
    ? allDatasetSeries.slice(-8)
    : allDatasetSeries;
  const rawSeries = datasetSeries.map((point) => point.value);
  const preview: SeriesPoint[] = datasetSeries.map((point) => ({
    step: point.step,
    period: point.period,
    actual: point.value,
  }));
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
        <div className="space-y-4">
          <Card
            title="Dataset and Forecast Controls"
          ...
          >
            <div className="space-y-4 text-sm">
              <label className="block text-gray-700 dark:text-gray-200">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    setTargetColumn(
                      defaultTargets[next.id] ?? numericColumns(next)[0] ?? "",
                    );
                    reset();
                  }}
              ...
              <label className="block text-gray-700 dark:text-gray-200">
                Learning rate: <b>{learningRate.toFixed(3)}</b>
                <input
                  type="range"
                  min={0.004}
                  max={0.05}
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  disabled={training}
                  onClick={train}
                  className="inline-flex min-h-10 items-center justify-center gap-2 rounded bg-blue-600 px-3 py-2 font-semibold text-white disabled:opacity-60"
                >
                  <Play size={14} />
                ...
                <button
                  ...
                  disabled={training}
                  onClick={reset}
                  className="inline-flex min-h-10 items-center justify-center gap-2 rounded border border-gray-300 px-3 py-2 font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-60 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700"
                >
                  <RotateCcw size={14} />
                  Reset
                </button>
          ...
          />
          <InfoBox type="success" title="Runtime">
            {status}
          </InfoBox>
        </div>
        <div className="space-y-4">
            ...
            <ResponsiveContainer width="100%" height={390}>
              ...
              >
                ...
                <Tooltip
                  ...
                  labelFormatter={(step) =>
                    displayChart.find((point) => point.step === Number(step))
                      ?.period ?? `Step ${step}`
                  }
                />
                <Legend />
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <Card title="Dataset Shape">
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="rounded bg-gray-50 p-2 dark:bg-gray-900">
                  <p className="text-gray-500">Numeric</p>
                  <p className="font-mono text-lg font-bold">
                    {availableTargets.length}
                  </p>
                </div>
                <div className="rounded bg-gray-50 p-2 dark:bg-gray-900">
                  <p className="text-gray-500">Lookback</p>
                  <p className="font-mono text-lg font-bold">{lookback}</p>
                </div>
                <div className="rounded bg-gray-50 p-2 dark:bg-gray-900">
                  <p className="text-gray-500">Horizon</p>
                  <p className="font-mono text-lg font-bold">{horizon}</p>
                </div>
              </div>
            </Card>
          ...
          <TrainingLossChart
            data={history}
            title="TensorFlow.js Training Loss"
            subtitle="Mean squared error reported at every recurrent training epoch."
            showAccuracy={false}
            height={280}
          />
          <div className="grid gap-4 lg:grid-cols-2">
            <Card title="Model Architecture">
              <div className="space-y-2 text-sm text-gray-700 dark:text-gray-200">
                <p>
                  <b>Input tensor:</b> batch x {lookback} steps x 1 feature
                </p>
                <p>
                  <b>Recurrent layer:</b> {meta.layer} with {units} hidden units
                </p>
                <p>
                  <b>Output:</b> one normalized next value
                </p>
                <p>
                  <b>Inference:</b> recursive {horizon}-step horizon
                </p>
              </div>
            </Card>
            <InfoBox type="info" title="Sequence Memory">
              {meta.gateNote}
            </InfoBox>
          </div>
        </div>
