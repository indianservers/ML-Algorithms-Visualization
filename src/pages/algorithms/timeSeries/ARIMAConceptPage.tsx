...
import ARIMAConceptApprovedPage from "./ARIMAConceptApprovedPage";
...
function makeSeries(noise: number, seed: number): SeriesPoint[] {
  return Array.from({ length: 120 }, (_, index) => {
    ...
    const pseudoNoise =
      Math.sin(index * 12.9898 + seed * 78.233) *
      Math.cos(index * 4.137 + seed) *
      noise;
    return {
      t: index,
      y: 0.3 * t + 2 * Math.sin(t) + 0.5 * Math.sin(3 * t) + pseudoNoise,
    };
  });
}
...
function linearArForecast(
  ...
  for (let index = Math.max(1, p); index < values.length; index++) {
    const ar =
      p === 0
        ? values[index - 1]
        : Array.from({ length: p }, (_, lag) => values[index - lag - 1]).reduce(
            (sum, value) => sum + value,
            0,
          ) / p;
    ...
    const ar =
      p === 0
        ...
        : Array.from(
            { length: p },
            (_, lag) =>
              history[history.length - lag - 1] ?? history.at(-1) ?? 0,
          ).reduce((sum, value) => sum + value, 0) / p;
    const ma =
      q === 0
        ? 0
        : residuals.slice(-q).reduce((sum, value) => sum + value, 0) / q;
...
function ARIMAConceptOriginalPage() {
  ...
  const diffChart = diffValues.map((value, index) => ({
    t: index + d,
    y: value,
  }));
  ...
  const forecastState = useMemo(() => {
    ...
    return {
      ...
      residuals: residuals
        .slice(-60)
        .map((value, index) => ({ t: index, error: value })),
  ...
  const combinedForecast = [
    ...
    ...forecastState.forecast.map((point) => ({
      ...point,
      observed: null,
      forecast: point.y,
    })),
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4">
      <PageHeader
        title="ARIMA"
        subtitle="Explore AR, differencing, MA residuals, ACF lags, and a browser-side ARIMA-style forecast."
        badge="Advanced"
        ...
        <div className="space-y-4">
          <Card title="ARIMA Controls">
            <div className="space-y-4 text-sm">
              ...
              <button
                ...
                className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded bg-blue-600 px-3 text-sm font-bold text-white hover:bg-blue-700"
              >
          ...
          <MetricsPanel
            ...
            metrics={[
              {
                label: "ADF Label",
                value: stationary ? "Stationary" : "Non-stationary",
              },
              {
                label: "Lag-1 ACF",
                value: acf(diffValues, 1)[1]?.acf ?? 0,
                format: "fixed4",
                color: stationary ? "green" : "red",
              },
              {
                label: "Residual Std",
                value: forecastState.residualStd,
                format: "fixed4",
              },
            ]}
          />
          <InfoBox type="info" title="Reading ARIMA">
            AR uses lagged values, I differences the series to make it more
            stationary, and MA feeds recent residual errors back into the
            forecast.
          </InfoBox>
        </div>
        <div className="space-y-4">
                  ...
                  <Bar dataKey="acf">
                    {acfData.map((item) => (
                      <Cell
                        key={item.lag}
                        fill={
                          item.lag > 0 && item.lag <= p ? "#2563eb" : "#94a3b8"
                        }
                      />
                    ))}
                  </Bar>
            ...
            <Card title="I(d): Differenced Series">
              <ResponsiveContainer width="100%" height={250}>
                <ComposedChart data={diffChart}>
                  ...
                  <Line
                    dataKey="y"
                    stroke={stationary ? "#059669" : "#dc2626"}
                    dot={false}
                  />
                </ComposedChart>
              </ResponsiveContainer>
...
export default function ARIMAConceptPage() {
  ...
  return new URLSearchParams(search).has("advanced") ? (
    <ARIMAConceptOriginalPage />
  ) : (
    <ARIMAConceptApprovedPage />
  );
}
