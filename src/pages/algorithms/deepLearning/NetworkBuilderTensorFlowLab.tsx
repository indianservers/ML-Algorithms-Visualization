...
const presets: Record<string, LayerConfig[]> = {
  ...
  Standard: [
    { id: "dense_1", type: "dense", units: 32, activation: "relu" },
    { id: "dense_2", type: "dense", units: 16, activation: "relu" },
  ],
  Deep: [16, 32, 32, 16].map((units, index) => ({
    id: `dense_${index}`,
    type: "dense",
    units,
  ...
  Dropout: [
    { id: "dense_1", type: "dense", units: 48, activation: "relu" },
    { id: "dropout_1", type: "dropout", rate: 0.2 },
    { id: "dense_2", type: "dense", units: 24, activation: "relu" },
  ],
  BatchNorm: [
    { id: "dense_1", type: "dense", units: 32, activation: "relu" },
    { id: "bn_1", type: "batchnorm" },
    { id: "dense_2", type: "dense", units: 16, activation: "relu" },
  ],
};
...
export default function NetworkBuilderPage() {
  ...
  const addLayer = (type: LayerConfig["type"]) => {
    setLayers((current) => [
      ...
      type === "dense"
        ...
        : type === "dropout"
          ? { id: `drop_${Date.now()}`, type, rate: 0.2 }
          : { id: `bn_${Date.now()}`, type },
    ]);
  };
  ...
  const drawBoundary = async (model: tf.LayersModel) => {
    ...
    for (let index = 0; index < points.length; index++) {
      ...
      ctx.fillStyle =
        p1 > 0.5 ? "rgba(37,99,235,0.24)" : "rgba(220,38,38,0.22)";
      ctx.fillRect(
        (gx / grid) * size,
        (gy / grid) * size,
        size / grid + 1,
    ...
    dataset.forEach((point) => {
      ctx.beginPath();
      ctx.arc(
        ((point.x + 1.2) / 2.4) * size,
        ((point.y + 1.2) / 2.4) * size,
        3,
        0,
        Math.PI * 2,
      );
  ...
  const train = async () => {
    ...
    const xs = tf.tensor2d(
      dataset.flatMap((point) => [point.x, point.y]),
      [dataset.length, 2],
    );
    const ys = tf.tensor2d(
      dataset.flatMap((point) => (point.label ? [0, 1] : [1, 0])),
      [dataset.length, 2],
    );
        ...
        onEpochEnd: async (epoch, logs) => {
          setHistory((current) => [
            ...current,
            {
              ...
              val_accuracy: Number(
                (
                  (logs?.val_acc as number | undefined) ??
                  (logs?.val_accuracy as number | undefined) ??
                  0
                ).toFixed(4),
              ),
            },
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4">
      <PageHeader
        ...
        subtitle="Design, train, visualize, and export neural network architectures directly in the browser."
      ...
      <div className="grid gap-6 xl:grid-cols-[300px_1fr_340px]">
        <div className="space-y-4">
          <Card title="Layer Palette">
            <div className="grid gap-2">
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded bg-blue-600 px-3 py-2 text-sm font-bold text-white"
              >
                <Plus size={14} /> Dense
              </button>
              <button
                onClick={() => addLayer("dropout")}
                className="rounded border border-gray-200 px-3 py-2 text-sm font-bold dark:border-gray-700"
              >
                Dropout
              </button>
              <button
                onClick={() => addLayer("batchnorm")}
                className="rounded border border-gray-200 px-3 py-2 text-sm font-bold dark:border-gray-700"
              >
                BatchNorm
              </button>
          ...
          <Card title="Dataset">
            <select
              value={datasetName}
              onChange={(event) =>
                setDatasetKind(event.target.value as DatasetName)
              }
              className="w-full rounded border border-gray-200 bg-white p-2 text-sm font-bold dark:border-gray-700 dark:bg-gray-900"
            >
            ...
            <button
              onClick={() => setDataset(makeDataset(datasetName))}
              className="mt-2 w-full rounded border border-gray-200 px-3 py-2 text-sm font-bold dark:border-gray-700"
            >
              Regenerate
            </button>
          </Card>
        ...
        <div className="space-y-4">
          <Card
            title="Architecture"
            subtitle={`Total: ${totalParams.toLocaleString()} params`}
          >
            <svg
              viewBox="0 0 760 230"
              className="h-[230px] w-full rounded bg-gray-50 dark:bg-gray-900"
            >
              {(
                [
                  ...
                  LayerConfig | { id: string; type: "input" | "output" }
                >
              ).map((layer, index, all) => {
                const x = 55 + index * (650 / Math.max(1, all.length - 1));
                const neurons =
                  layer.type === "dense"
                    ? Math.min(8, layer.units)
                  ...
                  <g key={layer.id}>
                    {index < all.length - 1 && (
                      <line
                        ...
                        x2={
                          55 +
                          (index + 1) * (650 / Math.max(1, all.length - 1)) -
                          35
                        }
                        y1={112}
                        y2={112}
                    ...
                    {Array.from({ length: neurons }, (_, neuron) => (
                      <circle
                        key={neuron}
                        cx={x}
                        cy={72 + neuron * (84 / Math.max(1, neurons - 1))}
                        r={9}
                        fill={
                          layer.type === "output"
                            ? "#059669"
                    ...
                    <text
                      ...
                      y={185}
                      textAnchor="middle"
                      className="fill-gray-700 text-[11px] font-bold dark:fill-gray-200"
                    >
                      {layer.type === "dense"
                        ? `Dense ${layer.units}`
                        : layer.type}
                    </text>
                    {layer.type === "dense" && layer.units > 8 && (
                      <text
                        x={x}
                        y={202}
                        textAnchor="middle"
                  ...
                  {layer.type === "dense" ? (
                    <>
                      ...
                      <input
                        ...
                        className="w-20 rounded border border-gray-200 bg-white p-1 dark:border-gray-700 dark:bg-gray-900"
                      />
                      <select
                          ...
                          setLayers((current) =>
                            current.map((item) =>
                              item.id === layer.id
                                ? {
                                    ...layer,
                                    activation: event.target.value as
                                      "relu" | "tanh" | "sigmoid",
                                  }
                                : item,
                            ),
                        ...
                        className="rounded border border-gray-200 bg-white p-1 dark:border-gray-700 dark:bg-gray-900"
                      >
                    ...
                    <>
                      <span className="font-bold">Dropout</span>
                      <input
                        type="number"
                        step={0.05}
                        min={0.05}
                        ...
                        onChange={(event) =>
                          setLayers((current) =>
                            current.map((item) =>
                              item.id === layer.id
                                ? { ...layer, rate: Number(event.target.value) }
                                : item,
                            ),
                          )
                        }
                    ...
                    <span className="font-bold">BatchNorm</span>
                  )}
                  <button
                    onClick={() =>
                      setLayers((current) =>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card title="Decision Boundary">
              <canvas
                ref={boundaryRef}
                className="aspect-square w-full rounded bg-white dark:bg-gray-950"
              />
            </Card>
              ...
              {history.length ? (
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={history}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line dataKey="accuracy" stroke="#2563eb" dot={false} />
                    <Line dataKey="val_accuracy" stroke="#059669" dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <p className="text-sm text-gray-500">
                  Train to see accuracy curves.
                </p>
              )}
            </Card>
          </div>
        ...
        <div className="space-y-4">
          <Card title="Train">
            <button
              disabled={training}
              onClick={() => void train()}
              className="inline-flex w-full items-center justify-center gap-2 rounded bg-emerald-600 px-3 py-3 text-sm font-bold text-white disabled:opacity-50"
            >
              <Play size={15} /> {training ? "Training..." : "Train Network"}
            </button>
              ...
              <button
                onClick={() =>
                  download(
                    "architecture.json",
                    JSON.stringify(
                      { layers, dataset: datasetName, history: history.at(-1) },
                      null,
                      2,
                    ),
                    "application/json",
                ...
                className="inline-flex items-center justify-center gap-2 rounded border border-gray-200 px-3 py-2 text-sm font-bold dark:border-gray-700"
              >
                <Download size={14} /> Architecture JSON
              </button>
              <button
                onClick={() => download("network.py", python, "text/x-python")}
                className="inline-flex items-center justify-center gap-2 rounded border border-gray-200 px-3 py-2 text-sm font-bold dark:border-gray-700"
              >
                <Download size={14} /> Python code
              </button>
            </div>
