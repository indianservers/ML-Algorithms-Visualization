...
function meanEmbedding(items: SupportExample[]) {
  ...
  return Array.from(
    { length },
    (_, index) =>
      items.reduce((sum, item) => sum + item.embedding[index], 0) /
...
export default function FewShotLearningPage() {
  ...
  const [classes, setClasses] = useState<FewShotClass[]>(() =>
  ...
  const [query, setQuery] = useState<{
    embedding: number[];
    scores: Array<{ classId: string; score: number }>;
  } | null>(null);
  const [status, setStatus] = useState(
    "Load MobileNet, add 1-5 examples per class, then run live cosine similarity.",
  );
  ...
  const startCamera = async () => {
    ...
    setStatus(
      "Camera is live. Capture support images or watch the query prediction update.",
    );
    if (timerRef.current) window.clearInterval(timerRef.current);
  ...
  const updateQuery = useCallback(async () => {
    ...
    const scores = classes
      .map((cls) => {
        ...
        return {
          classId: cls.id,
          score: items.length ? cosine(embedding, meanEmbedding(items)) : -1,
        };
      })
  ...
  const scatter = useMemo(() => {
    const points = examples.map((item) => ({
      x: item.embedding[0] ?? 0,
      y: item.embedding[1] ?? 0,
      classId: item.classId,
      kind: "support",
    }));
    if (query)
      points.push({
        x: query.embedding[0] ?? 0,
        y: query.embedding[1] ?? 0,
        classId: "query",
        kind: "query",
      });
    return points;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4">
      <PageHeader
        ...
        subtitle="Classify new images from 1-5 support examples per class using MobileNet embeddings and cosine similarity, with no retraining."
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        <div className="space-y-4">
          <Card title="Support Set">
            ...
            <button
              onClick={startCamera}
              className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded bg-blue-600 px-3 py-2 text-sm font-bold text-white"
            >
              <Camera size={14} /> Start Camera
            </button>
          </Card>
          {classes.map((cls) => {
            const classExamples = examples
            ...
            return (
              <Card key={cls.id} title={cls.name}>
                <input
                  ...
                  className="mb-2 w-full rounded border border-gray-200 bg-white p-2 text-sm font-bold dark:border-gray-700 dark:bg-gray-900"
                />
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => void addSupportFromVideo(cls.id)}
                    className="inline-flex items-center justify-center gap-2 rounded border border-gray-200 px-2 py-2 text-xs font-bold dark:border-gray-700"
                  >
                    <Camera size={13} /> Capture
                  </button>
                  <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded border border-gray-200 px-2 py-2 text-xs font-bold dark:border-gray-700">
        ...
        <div className="space-y-4">
          <Card title="Live Query">
            <video
              ...
              muted
              playsInline
              className="aspect-video w-full rounded-lg bg-gray-950 object-cover"
            />
            ...
            <div className="mt-4 rounded-2xl bg-gray-50 p-5 text-center dark:bg-gray-900">
              ...
              <p
                ...
                style={{
                  color: classes.find((cls) => cls.id === top?.classId)?.color,
                }}
              >
                {classes.find((cls) => cls.id === top?.classId)?.name ??
                  "Add support images"}
              </p>
              <p className="text-sm text-gray-500">
                {top
                  ? `${(top.score * 100).toFixed(1)}% cosine similarity`
                  : "Waiting for query frame"}
              </p>
            </div>
                  ...
                  <div key={score.classId}>
                    ...
                    <div className="h-3 rounded bg-gray-100 dark:bg-gray-800">
                      <div
                        ...
                        style={{
                          width: `${Math.max(0, score.score) * 100}%`,
              ...
              <ScatterChart>
                ...
                {classes.map((cls) => (
                  <Scatter
                    key={cls.id}
                    name={cls.name}
                    data={scatter.filter((item) => item.classId === cls.id)}
                    fill={cls.color}
                  />
                ))}
                <Scatter
                  name="Query"
                  data={scatter.filter((item) => item.kind === "query")}
                  fill="#111827"
                  shape="star"
                />
              </ScatterChart>
          ...
          <InfoBox type="info" title="Why This Works">
            {status} MobileNet was trained on a broad image corpus, so nearby
            1024-dimensional feature vectors often share visual meaning even for
            new classes.
          </InfoBox>
        </div>
      </div>
