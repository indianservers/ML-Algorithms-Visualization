...
import {
  ...
  type KMeansResult,
  type KMeansStep,
} from "../../../lib/algorithms/clustering/kmeans";
import {
...
const animalDatasetAssets = import.meta.glob(
  "../../../data/animals-dataset/**/*.{jpg,jpeg,png,webp}",
  {
    eager: true,
    import: "default",
    query: "?url",
...
function rowsToCsv(rows: PointRow[]) {
  return [
    "x,y,label",
    ...rows.map(
      (row) => `${row.x.toFixed(2)},${row.y.toFixed(2)},${row.label}`,
    ),
  ].join("\n");
}
...
function fitLine(rows: PointRow[]) {
  ...
  const numerator = rows.reduce(
    (sum, row) => sum + (row.x - meanX) * (row.y - meanY),
    0,
  );
  const denominator =
    rows.reduce((sum, row) => sum + (row.x - meanX) ** 2, 0) || 1;
...
function normalizeEmbedding(feature: number[]) {
  const magnitude =
    Math.sqrt(feature.reduce((sum, value) => sum + value ** 2, 0)) || 1;
  ...
  return centroids.reduce(
    (best, centroid, index) => {
      const distance = Math.sqrt(
        centroid.reduce(
          (sum, value, dim) => sum + (value - feature[dim]) ** 2,
          0,
        ),
      );
...
function silhouetteScore(
  ...
  const scores = values.map((value, index) => {
    ...
    const same = values.filter(
      (_, otherIndex) =>
        assignments[otherIndex] === ownCluster && otherIndex !== index,
    );
    const a = same.length
      ? same.reduce((sum, other) => sum + distance(value, other), 0) /
        same.length
      : 0;
    const otherMeans = Array.from({ length: clusterCount }, (_, cluster) => {
      ...
      const members = values.filter(
        (_, otherIndex) => assignments[otherIndex] === cluster,
      );
      return members.length
        ? members.reduce((sum, other) => sum + distance(value, other), 0) /
            members.length
        : Infinity;
    });
...
function calculateInertia(
  ...
  return values.reduce(
    (sum, value, index) =>
      sum + distance(value, centroids[assignments[index]]) ** 2,
    0,
  );
}
...
function pickInitialCentroids(
  ...
  while (centroids.length < count) {
    const distances = values.map((value) =>
      Math.min(...centroids.map((centroid) => distance(value, centroid) ** 2)),
    );
...
async function trainKMeansWithProgress(
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    ...
    assignments.forEach((cluster, index) => {
      ...
      values[index].forEach((value, dim) => {
...
export function KMeansAdvancedWorkbench() {
  ...
  const [imageStatus, setImageStatus] = useState(
    "Load the bundled animals dataset or your own image folders/files to cluster them with MobileNet embeddings.",
  );
  ...
  const pointRows = useMemo(() => {
    if (source === "mall") {
      return (
        mallCustomersDataset.data as {
          annual_income: number;
          spending_score: number;
        }[]
      ).map((row) => ({
        x: row.annual_income / 20 - 2,
        y: row.spending_score / 20 - 2,
        label: "customer",
      }));
    }
    if (source === "students") {
      return (
        studentMarksDataset.data as {
          student_id: number;
          study_hours: number;
          marks: number;
        }[]
    ...
    if (source === "synthetic") {
      return generateSyntheticBlobs(90, 3 + (seed % 2)).map((row) => ({
        x: row.x,
        y: row.y,
        label: `blob-${row.label + 1}`,
      }));
    }
  ...
  const pointX = useMemo(
    () => pointRows.map((point) => [point.x, point.y]),
    [pointRows],
  );
  const studentTrend = useMemo(
    () => fitLine(source === "students" ? pointRows : []),
    [pointRows, source],
  );
  const imageFeatures = useMemo(
    () => images.map((image) => normalizeEmbedding(image.feature)),
    [images],
  );
  const imageProjection = useMemo(
    () => projectEmbeddings(imageFeatures),
    [imageFeatures],
  );
  ...
  const pointResult = useMemo(
    () =>
      mode === "points" && canCluster
        ? kmeans(pointX, k, maxIter, init, seed)
        : null,
    [canCluster, init, k, maxIter, mode, pointX, seed],
  );
  const result = mode === "images" ? imageResult : pointResult;
  const activeStep =
    result?.steps[Math.min(step, Math.max(result.steps.length - 1, 0))] ??
    result?.steps[0];
  const displayedAssignments =
    mode === "images"
      ? result?.assignments
      : (activeStep?.assignments ?? result?.assignments);
  const displayedCentroids =
    mode === "images"
      ? result?.centroids
  ...
  const silhouette = useMemo(
    () =>
      activeX.length >= 4 && (mode === "points" || Boolean(result))
        ? Array.from(
            { length: Math.min(9, activeX.length - 1) },
            (_, index) => {
              const candidateK = index + 2;
              const candidate = kmeans(
                activeX,
                candidateK,
                Math.min(maxIter, 50),
                init,
  ...
  const suggestedK = silhouette.reduce(
    (best, item) => (item.silhouette > best.silhouette ? item : best),
    { k: 0, silhouette: -Infinity },
  ).k;
  const movement =
    result?.steps.map((item) => ({
      iteration: item.iteration,
      inertia: Number(item.inertia.toFixed(3)),
    })) ?? [];
  const studentPrediction = useMemo(() => {
    if (source !== "students") return null;
    const score = clamp(
      studentTrend.intercept + studentTrend.slope * studyHours,
      0,
      100,
    );
    const cluster = result?.centroids?.length
      ? nearestCentroid([studyHours, score], result.centroids).cluster
      : null;
  ...
  const predictedStudentPoint =
    source === "students" && studentPrediction
      ? [
          {
            x: studyHours,
  ...
  const clusterCounts = Array.from(
    { length: k },
    (_, cluster) =>
      plotData.filter((point) => point.cluster === cluster).length,
  );
  ...
  const loadAnimalDataset = async () => {
    ...
    if (!dataset.length) {
      setImageStatus(
        "No bundled animals dataset was found under src/data/animals-dataset.",
      );
      return;
    }
    setMode("images");
    ...
    setEmbeddingProgress({
      phase: "Loading and embedding images",
      current: 0,
    ...
    try {
      ...
      setImageStatus(
        `Embedding ${dataset.length} local animal images with TensorFlow.js MobileNet...`,
      );
      for (const [index, item] of dataset.entries()) {
        ...
        if (index % 4 === 0) {
          setImageStatus(
            `Embedded ${index + 1}/${dataset.length} animal images...`,
          );
        }
        setEmbeddingProgress({
          phase: "Loading and embedding images",
      ...
      setK(
        Math.min(
          5,
          Math.max(2, new Set(imported.map((item) => item.group)).size),
        ),
      );
      setStep(0);
      setImageStatus(
        `Ready: ${imported.length} image embeddings loaded. Click Train K-Means to cluster them.`,
      );
    } catch (error) {
      setImageStatus(
        error instanceof Error
          ? error.message
          : "Local animal dataset import failed.",
      );
    } finally {
  ...
  const loadFiles = async (fileList: FileList | null) => {
    ...
    try {
      const imported: ImageSample[] = [];
      setImageStatus(
        `Embedding ${files.length} image${files.length === 1 ? "" : "s"} with TensorFlow.js...`,
      );
      ...
      setStep(0);
      setImageStatus(
        `Loaded ${imported.length} image embeddings. Click Train K-Means to cluster them.`,
      );
    } catch (error) {
      setImageStatus(
  ...
  const trainImageClusters = async () => {
    ...
    setStep(0);
    setImageStatus(
      `Training K-Means on ${imageX.length} MobileNet image embeddings...`,
    );
    setTrainingProgress({
      phase: "Training K-Means clusters",
      current: 0,
    ...
    try {
      const trained = await trainKMeansWithProgress(
        imageX,
        k,
        maxIter,
        init,
      ...
      setTrainingProgress({
        phase: trained.converged ? "Training converged" : "Training complete",
        current: trained.steps.length || maxIter,
        total: trained.steps.length || maxIter,
      });
      setImageStatus(
        `Training complete: ${imageX.length} images clustered into ${k} groups. You can now run inference on a new image.`,
      );
    } catch (error) {
      setImageStatus(
        error instanceof Error ? error.message : "K-Means training failed.",
      );
    } finally {
  ...
  const addPoint = () => {
    ...
    next.push({
      x: Number((Math.cos(angle) * 3).toFixed(2)),
      y: Number((Math.sin(angle) * 3).toFixed(2)),
      label,
    });
    setSource("editable");
  ...
  const resetData = () => {
    ...
    if (mode === "images") {
      ...
      setTrainingProgress(null);
      setImageStatus(
        "Image samples cleared. Load the bundled animals dataset or your own folders/files.",
      );
    } else {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[380px_1fr]">
        <div className="space-y-4">
          <Card title="Experiment Mode">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <button
                ...
                className={`rounded px-3 py-2 font-semibold ${mode === "points" ? "bg-blue-600 text-white" : "border border-gray-200 dark:border-gray-700"}`}
              >
                Point Data
              </button>
              <button
                ...
                className={`rounded px-3 py-2 font-semibold ${mode === "images" ? "bg-blue-600 text-white" : "border border-gray-200 dark:border-gray-700"}`}
              >
                Image Data
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2 text-sm">
              ...
              >
                ...
                <span className="block text-xs font-normal opacity-80">
                  120 records, score check input, cluster view
                </span>
              </button>
              <button
                onClick={() => loadPointDataset("mall")}
                className={`rounded px-3 py-2 text-left font-semibold ${mode === "points" && source === "mall" ? "bg-blue-600 text-white" : "border border-gray-200 dark:border-gray-700"}`}
              >
                Mall customers
                <span className="block text-xs font-normal opacity-80">
              ...
              <button
                onClick={() => loadPointDataset("synthetic")}
                className={`rounded px-3 py-2 text-left font-semibold ${mode === "points" && source === "synthetic" ? "bg-blue-600 text-white" : "border border-gray-200 dark:border-gray-700"}`}
              >
                Synthetic blobs
                <span className="block text-xs font-normal opacity-80">
          ...
          {mode === "points" ? (
            <Card title="Point Dataset" icon={<Upload size={14} />}>
              <div className="space-y-3 text-sm">
                <select
                  value={source}
                  onChange={(event) => {
                    setSource(event.target.value as PointSource);
                    setStep(0);
                  }}
                ...
                >
                  <option value="editable">
                    Editable dog/car/horse-like points
                  </option>
                ...
                {source === "students" ? (
                  <div className="space-y-3 rounded border border-gray-200 p-3 dark:border-gray-700">
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <p className="text-lg font-bold text-blue-600">
                          {studentMarksDataset.data.length}
                        </p>
                      </div>
                      <div>
                    ...
                    <InfoBox type="success" title="Study Hours Check">
                      Estimated score:{" "}
                      {studentPrediction
                        ? studentPrediction.score.toFixed(1)
                        : "0.0"}{" "}
                      / 100
                      {studentPrediction?.cluster !== null &&
                      studentPrediction?.cluster !== undefined
                        ? ` - nearest cluster ${studentPrediction.cluster + 1}`
                        : ""}
                    </InfoBox>
                  </div>
                ) : (
                  <textarea
                    value={pointCsv}
                    onChange={(event) => {
                      setPointCsv(event.target.value);
                      setSource("editable");
                      setStep(0);
                    }}
                    rows={8}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    disabled={source === "students"}
                    onClick={addPoint}
                    className="inline-flex items-center justify-center gap-2 rounded border border-gray-200 px-3 py-2 font-semibold disabled:opacity-50 dark:border-gray-700"
                  >
                    <Plus size={14} /> Add Point
                  </button>
                  <button
                    onClick={() =>
                      downloadJson("kmeans-points.json", {
                        rows: pointRows,
                        k,
                        result,
                      })
                    }
                    className="inline-flex items-center justify-center gap-2 rounded border border-gray-200 px-3 py-2 font-semibold dark:border-gray-700"
                  >
                    Export Data
                  </button>
                </div>
            ...
            <Card
              title="Animal Image Clustering"
              icon={<ImageIcon size={14} />}
            >
              <canvas ref={canvasRef} className="hidden" />
              <div className="space-y-3 text-sm">
                <p className="text-xs text-gray-500">
                  Use the local bear/cat/cow/dog/horse folders, or load mixed
                  image folders. K-Means learns clusters from MobileNet
                  features; folder names are used to name the clusters after
                  training.
                </p>
                <div className="rounded border border-gray-200 p-3 dark:border-gray-700">
                  <label className="block text-xs font-semibold text-gray-500">
                    Images per animal folder:{" "}
                    <span className="font-mono text-blue-600">
                      {datasetLimit}
                    </span>
                  </label>
                  <input
                    ...
                    value={datasetLimit}
                    onChange={(event) =>
                      setDatasetLimit(Number(event.target.value))
                    }
                  ...
                  <button
                    ...
                    onClick={() => {
                      void loadAnimalDataset();
                    }}
                    className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded bg-blue-600 px-3 py-2 font-semibold text-white disabled:opacity-50"
                  >
                ...
                <label
                  className={`inline-flex w-full items-center justify-center gap-2 rounded px-3 py-2 font-semibold ${result && images.length ? "cursor-pointer bg-emerald-600 text-white" : "cursor-not-allowed bg-gray-200 text-gray-500 dark:bg-gray-800"}`}
                >
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(event) => {
                      void runInference(event.currentTarget.files);
                      event.currentTarget.value = "";
                    }}
                  />
                </label>
                <button
                  disabled={!result || !images.length}
                  onClick={() =>
                    downloadJson("kmeans-animal-clusters.json", {
                      k,
                  ...
                  className="inline-flex w-full items-center justify-center gap-2 rounded border border-gray-200 px-3 py-2 font-semibold disabled:opacity-50 dark:border-gray-700"
                >
                  Export Clusters
                </button>
                <button
                  ...
                  className="inline-flex w-full items-center justify-center gap-2 rounded border border-gray-200 px-3 py-2 font-semibold dark:border-gray-700"
                >
                  <Trash2 size={14} /> Clear Images
                </button>
                <InfoBox
                  type={images.length ? "success" : "info"}
                  title="Image Runtime"
                >
                  {loadingImages ? "Embedding images..." : imageStatus}
                </InfoBox>
              </div>
            </Card>
          )}
          ...
          <Card title="K-Means Controls">
            <div className="space-y-4 text-sm">
              <label className="block text-xs font-semibold text-gray-500">
                K: <span className="font-mono text-blue-600">{k}</span>
              </label>
              <input
                type="range"
                min={2}
              ...
              <select
                value={init}
                onChange={(event) => {
                  setInit(event.target.value as "random" | "kmeans++");
                  setStep(0);
                }}
                className="w-full rounded border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-900"
              >
              ...
              <label className="block text-xs font-semibold text-gray-500">
                Max iterations:{" "}
                <span className="font-mono text-blue-600">{maxIter}</span>
              </label>
              <input
                type="range"
                min={3}
          ...
          <Card title="Training Controls">
            <div className="flex gap-2">
              {mode === "images" ? (
                <button
                  disabled={!canCluster || loadingImages || trainingImages}
                  onClick={() => {
                    void trainImageClusters();
                  }}
                  className="flex flex-1 items-center justify-center gap-2 rounded bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"
                >
                  <Play size={14} /> Train K-Means
                </button>
              ) : (
                <button
                  disabled={!result}
            ...
            <div className="mt-3 space-y-2">
              ...
              <p className="text-xs text-gray-500">
                {mode === "images"
                  ? result
                    ? `Trained iterations: ${result.steps.length}`
                    : "Load images, then train K-Means to create clusters."
          ...
          <MetricsPanel
            ...
            metrics={[
              {
                label: "Inertia/SSE",
                value: result?.inertia ?? 0,
                format: "fixed4",
                color: "blue",
              },
              {
                label: mode === "images" ? "Image Embeddings" : "Samples",
                value: activeX.length,
                format: "number",
              },
        ...
        <div className="space-y-4">
            ...
            {canShowClusters ? (
              <ResponsiveContainer width="100%" height={390}>
                <ScatterChart>
                  ...
                  {predictedStudentPoint.length > 0 && (
                    <Scatter
                      name="Your study-hours check"
                      data={predictedStudentPoint}
                      fill="#111827"
                      shape="diamond"
                    />
                  )}
              ...
              <div className="rounded-lg border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500 dark:border-gray-700">
                {mode === "images" && images.length >= k
                  ? "Click Train K-Means to create clusters."
            ...
            <Card title="Image Cluster Gallery">
                ...
                <div className="mb-4 flex items-center gap-3 rounded border border-emerald-200 bg-emerald-50 p-3 text-sm dark:border-emerald-900 dark:bg-emerald-950/30">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400">
                      {inference.name} nearest-cluster distance{" "}
                      {inference.distance.toFixed(3)} - similarity score{" "}
                      {(inference.similarity * 100).toFixed(1)}%
                    </p>
                  </div>
                </div>
              )}
              {result ? (
                  ...
                  {Array.from({ length: k }, (_, cluster) => (
                    ...
                    >
                      <p
                        ...
                        style={{ color: colors[cluster % colors.length] }}
                      >
                        Cluster {cluster + 1} ({clusterCounts[cluster] ?? 0})
                      </p>
                      <p className="mb-2 text-[11px] font-semibold text-gray-500">
                        {clusterLabels[cluster]?.label ?? "unlabeled"}{" "}
                        {clusterLabels[cluster]?.total
                          ? `- ${(clusterLabels[cluster].purity * 100).toFixed(0)}% match`
                          : ""}
                      </p>
                      <div className="grid grid-cols-3 gap-1">
                        {images
                          .map((image, index) => ({
                            image,
                            cluster: displayedAssignments?.[index] ?? -1,
                          }))
                ...
                <div className="rounded border border-dashed border-gray-200 p-6 text-center text-sm text-gray-500 dark:border-gray-700">
                  {images.length
                    ? "Images are embedded. Click Train K-Means to build the clusters."
                    : "Load animal images to begin."}
                </div>
              )}
            </Card>
          )}
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            ...
            <Card title="Silhouette Score by k">
              ...
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                Silhouette compares within-cluster distance against the nearest
                other cluster. Its peak may disagree with the elbow, which is
                the useful lesson.
              </p>
            </Card>
          </div>
          ...
          <InfoBox type="success" title="Real Logic Cross-Check">
            Numeric mode clusters editable x/y rows. Image mode embeds your
            images with TensorFlow.js MobileNet, runs K-Means on the
            high-dimensional embedding vectors, then projects them to 2D only
            for visualization.
          </InfoBox>
        </div>
      </div>
