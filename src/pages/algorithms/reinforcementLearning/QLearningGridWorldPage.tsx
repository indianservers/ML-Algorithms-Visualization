...
import QLearningGridWorldApprovedPage from "./QLearningGridWorldApprovedPage";
...
function smoothRewards(
  rewards: number[],
  w = 20,
): { episode: number; reward: number; smoothed: number }[] {
...
// ─────────────────────────────────────────────────────────────────────────
function QLearningGridWorldOriginalPage() {
  ...
  const finalReward = result
    ? result.episodeRewards[result.episodeRewards.length - 1].toFixed(2)
    : "-";
  const avgReward = result
    ? mean(result.episodeRewards.slice(-50)).toFixed(2)
    : "-";

  // Max Q per cell
  const maxQPerCell = useCallback(
              ...
              {result && (
                <div className="space-y-2">
                  ...
                  <label className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-gray-700">
                    <input
                      type="checkbox"
                      checked={showValueMap}
                      onChange={(event) =>
                        setShowValueMap(event.target.checked)
                      }
          ...
          {result && (
            <MetricsPanel
              ...
              metrics={[
                ...
                {
                  label: "States",
                  value: Object.keys(result.qTable).length,
                  format: "number",
                },
              ]}
            />
        ...
        <div className="space-y-4">
          <Card title="Grid World (5×5)">
                ...
                row.map((cell, c) => {
                  ...
                  return (
                    ...
                    >
                      {/* Agent marker */}
                      {isAgent && (
                        <span className="absolute top-0.5 right-0.5 text-lg z-10">
                          🤖
                        </span>
                      )}
                      {/* Policy arrow */}
                      ...
                      {/* Coords */}
                      <span className="text-[8px] opacity-40 font-mono absolute bottom-0.5 left-1">
                        {r},{c}
                      </span>
                    </div>
                  );
            ...
            <div className="mt-3 grid grid-cols-3 gap-x-4 gap-y-1 text-[10px] text-gray-500">
              {(["empty", "wall", "goal", "start", "penalty"] as Cell[]).map(
                (t) => (
                  <div key={t} className="flex items-center gap-1">
                    <span
                      className={`w-3 h-3 rounded ${CELL_COLORS[t]} border border-gray-300 inline-block`}
                    />
          ...
          {result && (
            <InfoBox type="success" title="Training complete!">
              Policy arrows show the best action in each non-wall cell. Toggle
              the value map to color each state by V(s)=max Q(s,a), then replay
              the greedy policy step by step.
            </InfoBox>
          )}
        </div>
              ...
              <Card
                ...
                actions={
                  ...
                  >
                    ...
                    <ChevronRight
                      size={11}
                      className={`transition-transform ${showQTable ? "rotate-90" : ""}`}
                    />
                  </button>
                }
              >
                        ...
                        {Object.entries(result.qTable)
                          ...
                          .map(([state, vals]) => (
                            ...
                            >
                              ...
                              {(
                                ["up", "down", "left", "right"] as Action[]
                              ).map((a) => (
                                <td
                                  key={a}
                                  className={`px-2 py-1 text-center ${
                                    result.policy[state] === a
                ...
                {!showQTable && (
                  <p className="text-xs text-gray-400 text-center py-2">
                    Click "Show" to expand Q-table (
                    {Object.keys(result.qTable).length} states × 4 actions)
                  </p>
                )}
              </Card>
            </>
        ...
        sections={[
          {
            ...
            content: (
              <div className="space-y-1">
                <p>
                  Q-Learning is a model-free, off-policy algorithm. It maintains
                  a Q-table mapping (state, action) pairs to expected cumulative
                  rewards.
                </p>
          ...
          },
          {
            ...
            content: (
              <ul className="list-disc ml-4 space-y-1">
                <li>
                  <strong>α (learning rate)</strong>: how fast Q-values update.
                  Too high → unstable; too low → slow.
                </li>
                <li>
                  <strong>γ (discount)</strong>: future reward weight. Near 1 =
                  long-sighted; near 0 = greedy.
                </li>
                <li>
                  <strong>ε (epsilon)</strong>: exploration rate. Decaying
                  epsilon is common in practice.
                </li>
              </ul>
            ),
          },
...
export default function QLearningGridWorldPage() {
  return new URLSearchParams(window.location.search).has("advanced") ? (
    <QLearningGridWorldOriginalPage />
  ) : (
    <QLearningGridWorldApprovedPage />
  );
}
