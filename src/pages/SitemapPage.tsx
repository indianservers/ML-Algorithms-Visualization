    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
      <div className="mb-6 border-b border-gray-200 pb-5 dark:border-gray-800">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-600 dark:text-gray-300">
          Public index of canonical pages available to crawlers. The XML version
          is available at{" "}
          <a
            href="/sitemap.xml"
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        <div>
          <p className="text-sm font-bold text-gray-900 dark:text-white">
            {routes.length} canonical URLs
          </p>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Generated from the same route metadata used by the app head tags.
          </p>
        </div>
        <a
          href="/sitemap.xml"
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-800">
          {routes.map((route) => (
            <Link
              key={route.path}
              to={route.path}
              className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 px-4 py-3 text-sm hover:bg-blue-50 dark:hover:bg-blue-950/20"
            >
...
export default function SitemapPage() {
  ...
  return new URLSearchParams(location.search).get("advanced") === "1" ? (
    ...
    <PlatformApprovedPage page="sitemap" />
  );
}
