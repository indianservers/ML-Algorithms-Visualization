...
const typeUseCases: Record<Dataset["type"], string> = {
  regression:
    "Predict continuous outcomes, compare residuals, and test feature importance.",
  classification:
    "Train class predictors, inspect confusion matrices, ROC curves, and explainability.",
  clustering:
    "Segment records, compare cluster shapes, and explore unsupervised structure.",
  timeSeries:
    "Smooth trends, forecast sequential signals, and inspect anomalies over time.",
  nlp: "Vectorize text, classify labels, and compare bag-of-words or TF-IDF behavior.",
  recommendation:
    "Explore user-item preference patterns and collaborative filtering demos.",
};

const inferTarget = (dataset: Dataset) => {
  if (dataset.type === "clustering" || dataset.type === "recommendation")
    return undefined;
...
function buildDatasetPresets(): DatasetPreset[] {
  ...
  return allSampleDatasets.map((dataset) => {
    ...
    algorithms.forEach((algorithm) => {
      const suggestions = getAlgorithmDatasetSuggestions(
        algorithm.route,
        algorithm.category,
      );
    ...
    if (targetVariables.size === 0 && fallbackTarget)
      targetVariables.add(fallbackTarget);
    ...
    return {
      ...
      compatibleAlgorithms: compatibleAlgorithms
        .filter(
          (algorithm, index, items) =>
            items.findIndex((item) => item.route === algorithm.route) === index,
        )
        .slice(0, 10),
    };
  });
...
function DatasetLibraryPageOriginal() {
  const [query, setQuery] = React.useState("");
  const [typeFilter, setTypeFilter] = React.useState<"All" | Dataset["type"]>(
    "All",
  );
  ...
  const filteredPresets = presets.filter((preset) => {
    ...
    const matchesType =
      typeFilter === "All" || preset.dataset.type === typeFilter;
    const matchesSearch =
      !search ||
      preset.dataset.name.toLowerCase().includes(search) ||
      preset.dataset.description.toLowerCase().includes(search) ||
      preset.dataset.columns.some((column) =>
        column.toLowerCase().includes(search),
      ) ||
      preset.compatibleAlgorithms.some((algorithm) =>
        algorithm.label.toLowerCase().includes(search),
      );
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-950 dark:text-white">
              Dataset Preset Library
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-600 dark:text-gray-300">
              Central index of sample datasets, target variables, use cases,
              columns, and compatible algorithm modules.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            ...
            <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-lg font-bold text-gray-950 dark:text-white">
                {new Set(presets.map((item) => item.dataset.type)).size}
              </p>
            ...
            <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-900">
              <p className="text-lg font-bold text-gray-950 dark:text-white">
                {presets.reduce(
                  (sum, item) => sum + item.compatibleAlgorithms.length,
                  0,
                )}
              </p>
      ...
      <div className="mb-5 grid gap-3 lg:grid-cols-[1fr_auto]">
        ...
        <select
          value={typeFilter}
          onChange={(event) =>
            setTypeFilter(event.target.value as "All" | Dataset["type"])
          }
      ...
      <div className="grid gap-4">
        {filteredPresets.map(
          ({ dataset, targetVariables, compatibleAlgorithms }) => (
            <article
              key={dataset.id}
              ...
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(280px,420px)]">
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="rounded-lg bg-blue-50 px-3 py-2 text-sm text-blue-900 dark:bg-blue-950/30 dark:text-blue-100">
                      {typeUseCases[dataset.type]}
                    </p>
                  </div>
                  <div>
                    <p className="mb-2 text-xs font-bold uppercase tracking-wide text-gray-500">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {targetVariables.length > 0 ? (
                        targetVariables.map((target) => (
                          <code
                            key={target}
                            className="rounded bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-700 dark:bg-gray-800 dark:text-gray-200"
                          >
                            {target}
                          </code>
                        ))
                      ) : (
                        <span className="rounded bg-gray-100 px-2 py-1 text-xs font-semibold text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                          Unsupervised / matrix data
                        </span>
                      )}
                    </div>
                  </div>
                    ...
                    {compatibleAlgorithms.length > 0 ? (
                      compatibleAlgorithms.map((algorithm) => (
                        <Link
                          key={algorithm.route}
                          to={algorithm.route}
                          className="flex min-h-10 items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm hover:border-blue-300 hover:bg-blue-50 dark:border-gray-700 dark:hover:bg-blue-950/30"
                        >
                      ...
                      <p className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
                        Available from the Dataset Manager.
                      </p>
                    )}
                  </div>
                </div>
...
export default function DatasetLibraryPage() {
  ...
  return new URLSearchParams(location.search).get("advanced") === "1" ? (
    <DatasetLibraryPageOriginal />
  ) : (
    <PlatformApprovedPage page="datasets" />
  );
}
