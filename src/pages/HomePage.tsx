...
const categories = [
  ...
  },
  {
    ...
    title: "Clustering",
    description:
      "K-Means, DBSCAN, Hierarchical, GMM with step-by-step animation",
...
function HomePageOriginal() {
  const [recentExperiments, setRecentExperiments] = React.useState<
    Experiment[]
  >([]);
  ...
  const coverageSegments = [
    {
      label: "Implemented",
      count: summary.counts.Implemented,
      color: "bg-green-500",
    },
    {
      label: "Educational",
      count: summary.counts.Educational,
      color: "bg-amber-500",
    },
  ...
  const categoryCoverage = navigationData.map((category) => {
    ...
    const implemented = items.filter(
      (item) => getImplementationStatus(item.route) === "Implemented",
    ).length;
  ...
  const learningPaths = [
    {
      ...
      description:
        "Start with the core ideas, visual intuition, and essential metrics.",
    },
    {
      title: "Intermediate Path",
      level: "Intermediate" as const,
      description:
        "Move into regularization, validation, preprocessing, and richer models.",
    },
    {
      title: "Advanced Path",
      level: "Advanced" as const,
      description:
        "Explore ensembles, kernels, deep learning, and probabilistic methods.",
    },
  ];
  const scaffoldItems = summary.items
    .filter((item) => getImplementationStatus(item.route) === "Scaffold")
    .slice(0, 12);
    ...
    loadExperiments()
      .then((items) =>
        setRecentExperiments(
          items.sort((a, b) => b.createdAt - a.createdAt).slice(0, 5),
        ),
      )
  ...
  React.useEffect(() => {
    ...
    return () =>
      window.removeEventListener("ml:learner-progress-changed", refresh);
  }, []);
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mb-6">
          Learn, experiment, and compare 100+ ML algorithms with fully
          interactive visualizations- all running directly in your browser. No
      ...
      <div className="mb-10 grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900 lg:col-span-3">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                <Trophy size={18} /> Curriculum Dashboard
              </h2>
              <p className="text-sm text-gray-500">
                Progress, quizzes, achievements, and category completion are
          ...
          <div className="mb-4 h-2 overflow-hidden rounded bg-gray-100 dark:bg-gray-800">
            <div
              ...
              style={{
                width: `${learningStats.total ? (learningStats.completed / learningStats.total) * 100 : 0}%`,
              }}
            />
          </div>
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-lg bg-blue-50 p-3 text-blue-900 dark:bg-blue-950/30 dark:text-blue-100">
              ...
              <p className="mt-1 text-xl font-bold">
                {Math.round(learningStats.quizAverage * 100)}%
              </p>
            </div>
            <div className="rounded-lg bg-green-50 p-3 text-green-900 dark:bg-green-950/30 dark:text-green-100">
              ...
              <p className="mt-1 text-xl font-bold">
                {
                  learningStats.achievements.filter((item) => item.earned)
                    .length
                }
            ...
            <div className="rounded-lg bg-amber-50 p-3 text-amber-900 dark:bg-amber-950/30 dark:text-amber-100">
              ...
              <p className="mt-1 text-xl font-bold">
                {learningStats.recentNotes.length}
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {learningStats.achievements.map((item) => (
                  <span
                    key={item.id}
                    className={`rounded-full px-3 py-1 text-xs font-semibold ${item.earned ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}
                  >
                    {item.label}
                  </span>
                ))}
            ...
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {learningStats.categories.slice(0, 8).map((item) => (
                  <div
                    key={item.category}
                    className="rounded border border-gray-200 px-3 py-2 text-xs dark:border-gray-700"
                  >
        ...
        >
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
                <Activity size={18} /> Suite Coverage
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Transparent status across the algorithm library.
              </p>
            </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {categoryCoverage.slice(0, 12).map((item) => (
              <div
                key={item.category}
                className="rounded border border-gray-200 bg-gray-50 px-3 py-2 text-xs dark:border-gray-700 dark:bg-gray-800"
              >
                <div className="mb-1 flex justify-between gap-2">
                  <span className="truncate font-semibold text-gray-700 dark:text-gray-200">
                    {item.category}
                ...
                <div className="h-1.5 overflow-hidden rounded bg-gray-200 dark:bg-gray-700">
                  <div
                    className="h-full bg-green-500"
                    style={{
                      width: `${item.total ? (item.implemented / item.total) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                {items.map((item, index) => (
                  <Link
                    key={item.route}
                    to={item.route}
                    className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-700 hover:border-blue-300 hover:bg-blue-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-blue-900/20"
                  >
                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-white text-[10px] font-bold text-gray-500 dark:bg-gray-900 dark:text-gray-400">
                      {index + 1}
                    </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
        {visibleCategories.map((cat, i) => (
          ...
          >
            <div
              className={`w-10 h-10 rounded-xl bg-gradient-to-br ${cat.color} flex items-center justify-center text-white mb-3`}
            >
              {cat.icon}
            </div>
      ...
      <div className="mb-8 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          <h2 className="mb-3 flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
            <Clock size={17} /> Continue Where You Left Off
          </h2>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {recentItems.length === 0 ? (
              <p className="text-sm text-gray-500">
                Open any algorithm route and it will appear here.
              </p>
            ) : (
              recentItems.map(
                (item) =>
                  item && (
                    <Link
                      key={item.route}
                      to={item.route}
                      className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-medium text-gray-700 hover:border-blue-300 hover:bg-blue-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-blue-900/20"
                    >
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          <h2 className="mb-3 flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
            <Database size={17} /> Recently Saved Experiments
          </h2>
          <div className="space-y-2">
            {recentExperiments.length === 0 ? (
              <p className="text-sm text-gray-500">
                Saved experiment cards will appear after you save from an
      ...
      {scaffoldItems.length > 0 && (
        <div className="mb-8 rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/20">
          <h2 className="mb-3 text-lg font-bold text-red-900 dark:text-red-100">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {scaffoldItems.map((item) => (
              <Link
                key={item.route}
                to={item.route}
                className="flex items-center justify-between rounded-lg border border-red-200 bg-white/70 px-3 py-2 text-xs font-semibold text-red-800 hover:bg-white dark:border-red-900 dark:bg-red-950/40 dark:text-red-200"
              >
                <span className="truncate">{item.label}</span>
                <Badge type="Scaffold" />
              </Link>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            ...
            },
            {
              ...
              to: "/ml/supervised/logistic-regression",
              badge: "Beginner",
            },
            {
              label: "DBSCAN",
              to: "/ml/clustering/dbscan",
              badge: "Intermediate",
            },
            { label: "TF-IDF", to: "/ml/nlp/tf-idf", badge: "NLP" },
            {
              label: "Q-Learning",
              to: "/ml/reinforcement-learning/q-learning-grid-world",
              badge: "Interactive",
            },
          ].map((item, i) => (
            <Link
