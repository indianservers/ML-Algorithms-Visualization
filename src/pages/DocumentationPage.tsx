...
function DocumentationPageOriginal() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-950 dark:text-white">
              Mega ML Algorithms Suite Documentation
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-600 dark:text-gray-300">
              A complete route index for every learning module, browser lab,
              implementation status, and search metadata entry.
            </p>
          </div>
      ...
      <div className="mb-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Link
          to="/dataset-library"
          className="flex min-h-20 items-center gap-3 rounded-lg border border-gray-200 bg-white p-4 text-sm font-semibold text-gray-800 hover:border-blue-300 hover:bg-blue-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:hover:bg-blue-950/30"
        >
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {group.items.map((item) => (
                <Link
                  key={item.route}
                  to={item.route}
                  className="block rounded-lg border border-gray-200 bg-white p-4 hover:border-blue-300 hover:bg-blue-50/70 dark:border-gray-700 dark:bg-gray-900 dark:hover:bg-blue-950/20"
                >
...
export default function DocumentationPage() {
  ...
  return new URLSearchParams(location.search).get("advanced") === "1" ? (
    <DocumentationPageOriginal />
  ) : (
    <PlatformApprovedPage page="documentation" />
  );
}
