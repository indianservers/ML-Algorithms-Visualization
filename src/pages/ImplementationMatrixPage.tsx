  ...
  const header = (key: SortKey, label: string) => (
    ...
    >
      ...
      <span className="text-[10px] text-gray-400">
        {sortKey === key ? (ascending ? "asc" : "desc") : "sort"}
      </span>
    </button>
  );
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4">
      <PageHeader
        ...
        subtitle="Quality gate view for every algorithm route and its current implementation status."
        badge="Advanced"
            ...
            <tbody>
              {sortedItems.map((item) => (
                <tr
                  key={item.route}
                  className={
                    getImplementationStatus(item.route) === "Scaffold"
                ...
                >
                  ...
                  <td className="border border-gray-200 p-2 font-semibold dark:border-gray-700">
                    {item.label}
                  </td>
                  <td className="border border-gray-200 p-2 dark:border-gray-700">
                  ...
                  <td className="border border-gray-200 p-2 font-mono dark:border-gray-700">
                    <Link
...
export default function ImplementationMatrixPage() {
  ...
  return new URLSearchParams(location.search).get("advanced") === "1" ? (
    <ImplementationMatrixPageOriginal />
  ) : (
    <PlatformApprovedPage page="matrix" />
  );
}
